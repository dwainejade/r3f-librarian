...
export default function FirstPersonCamera({ position = [0, 1.6, 5] }) {
  ...
  // Safe Center-screen raycasting
  useFrame(({ raycaster, scene }) => {
    if (!raycaster || !raycaster.layers) return; // CRITICAL CRASH SAFETY SHIELD

    raycaster.setFromCamera({ x: 0, y: 0 }, raycaster.camera);
    raycaster.layers.set(0); 
    raycaster.far = 3.05; // 10 feet max range

    const intersects = raycaster.intersectObjects(scene.children, true);

    if (intersects.length > 0) {
      // Find objects that contain our physics body userData tags
      const hit = intersects.find(h => h.object.parent?.userData?.bookName || h.object.userData?.bookName);
      if (hit) {
        const name = hit.object.parent?.userData?.bookName || hit.object.userData?.bookName;
